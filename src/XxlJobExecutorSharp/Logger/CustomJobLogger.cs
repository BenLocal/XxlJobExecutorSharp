...
namespace XxlJobExecutorSharp.Logger
{
    public class CustomJobLogger : ILogger
    {
        ...
        public void Log<TState>(LogLevel logLevel, EventId eventId, TState state, Exception exception, Func<TState, Exception, string> formatter)
        {
            ...
            if (scopes.Exists(x => x.Properties.ContainsKey(XxlJobConstant.LOGGER_SCOPE_JOBAREA_KEY) &&
                x.Properties[XxlJobConstant.LOGGER_SCOPE_JOBAREA_KEY]?.ToString() == XxlJobConstant.LOGGER_SCOPE_JOBAREA_VALUE))
            {
                var scope = scopes.FirstOrDefault(x => x.Properties.ContainsKey(XxlJobConstant.LOGGER_SCOPE_JOBAREA_KEY) &&
                    x.Properties[XxlJobConstant.LOGGER_SCOPE_JOBAREA_KEY]?.ToString() == XxlJobConstant.LOGGER_SCOPE_JOBAREA_VALUE);
                if (scope.Properties.TryGetValue(XxlJobConstant.LOGGER_SCOPE_JOBID_KEY, out var jobId) &&
                    scope.Properties.TryGetValue(XxlJobConstant.LOGGER_SCOPE_JOBLOGID_KEY, out var jobLogId))
                {
                    _provider.JobLoggerStore.Save(new Entity.JobLoggerInfo()
                    {
